...
try:
    ...
    Himage = Image.open(os.path.join(picdir, '7in3f3.bmp')).convert("P").quantize(colors=7, method=Image.Quantize.MEDIANCUT, kmeans=2, palette="ADAPTIVE", dither=Image.Dither.FLOYDSTEINBERG)
    epd.display(epd.getbuffer(Himage))
    time.sleep(3)
